...
Hardcode the axis conventions used in the WRemnants nonprompt background
estimation:
  - y-axis: "relIso"  (bin 0 = tight/signal, bin 1 = loose/sideband)
...
def _isomtmt_regions(indata, channel_name):
    """Return (mt_signal_idx, mt_sideband_idx) from the channel's mt axis size."""
...
class ABCDIsoMT(ABCD):
    ...
    Uses the last two mt bins as signal / sideband and relIso bins 0 / 1 as
...
class ExtendedABCDIsoMT(ExtendedABCD):
    ...
    Uses the last three mt bins as signal / sideband / extra-sideband and
    ...
    def __init__(self, indata, abcd_process, channel_name, **kwargs):
        ...
        channel_Ax = {channel_name: {"relIso": 0, "mt": mt_xsb}}
        channel_Bx = {channel_name: {"relIso": 1, "mt": mt_xsb}}
        channel_A = {channel_name: {"relIso": 0, "mt": mt_sb}}
        channel_B = {channel_name: {"relIso": 1, "mt": mt_sb}}
        channel_C = {channel_name: {"relIso": 0, "mt": mt_s}}
        channel_D = {channel_name: {"relIso": 1, "mt": mt_s}}
        super().__init__(
            indata,
            abcd_process,
            channel_A,
