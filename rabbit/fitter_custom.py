...
class Fitter:
    def __init__(self, indata, options, do_blinding=False):
        ...
        if self.do_blinding:
            ...
            # self._blinding_offsets_amplitude = tf.Variable(
            #     tf.zeros([self.indata.ncustom], dtype=self.indata.dtype),
            #     trainable=False,
            #     name="offset_amplitude",
            # )
            self.init_blinding_values(options.unblind)

        ### okay so i want an amplitude for every systematic that i multiply it by. not quite sure how that fits into this framework
        ...
        ### new ###
        nwilson = 4
        wilson_coeffs = tf.zeros([nwilson], dtype=float)### not exactly sure what type i want
        xcustom = tf.concat([xdefault, wilson_coeffs], axis = 0)
        self.x = tf.Variable(xcustom, trainable=True, name="x")
    ...
    def get_blinded_theta(self):
        theta = self.x[self.npoi : -self.custom_params]
        if self.do_blinding:
    ...
    def get_blinded_custom(self):
        amplitude = self.x[self.custom_params]
    ...
    def _compute_yields_noBBB(self, compute_norm=False, full=True):
        ...
        custom_params = self.get_blinded_custom_params()
        ...
        # scaling custom model
        
        ### okay im going to attempt to access the mass and time so that i can write the function
        channel_info = self.indata.channel_info[channel]

        channel= {
            "axes": self.term.channel_axes,
            "flow": channel_info.get("flow", False),
            "processes": self.processes if len(self.processes) else self.indata.procs,
            }
        
        ### need to figure out how to make this an unfolded version so i can access the mass and time bins because i don't have this in rapidity yet
        #### i'd need to explicitly choose a channel
        ch_axes = self.channels[channel]["axes"]
        ### need to get the length of each axis
        for ax in ch_axes:
            ax_len = 
        ### get bin centers from the index number
        
        
        def custom_function(custom_params, mass, rapidity, time):
            return x
        
        liv = custom_function(custom_params, mass, rapidity, time)
        normcentral[signal] = normcentral[signal] * liv
