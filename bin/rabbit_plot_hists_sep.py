...
def make_plot(
    ...
    for axis_name in ["pt", "eta"]:
        ...
        if axis_name == "eta":
            other_axis = "pt"
        xlabel = f"{other_axis}_probe bin" ### for now am going to hard code this in
        ...
        for j in range(len(h_inclusive[{f"{other_axis}_probe": 0}].values())):
            ### this is already data, need it to be mc
            hep.histplot(
                ...
                yerr=False,
                color=f"C{j}",
                label = f"{axis_name}_probe bin {j}",
                binwnorm=binwnorm,
                ax=ax1,
                zorder = 1,
                flow="none",
            ...
            std = np.sqrt(h_inclusive[{f"{axis_name}_probe": j}].variances()) / binwidth
